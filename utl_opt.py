...
def utiliy_opt_2_users(r, u_func, du_func):
    ...
    while high - low > 1:
        mid = low + (high - low) / 2
        _, r_tot = assign_from_midd(r, idx_sort, mid)
        if du_func[0](r_tot[0]) / du_func[1](r_tot[1]) < ratio[mid]:
            low, high = low, mid
        else:
            low, high = mid, high

    assign_low, r_tot_low = assign_from_midd(r, idx_sort, low)
    assign_high, r_tot_high = assign_from_midd(r, idx_sort, high)
...
def assign_from_midd(r, idx_sort, mid):
